...
async function requireSupabaseSession(client: NonNullable<typeof supabase>) {
  ...
  throw new Error('Supabase ?ëª„ì¬†??ë¾????ë¾???¿ì”¤??ï§ì•¹???½ë„—????•ì¾­????»ìªŸ ?ëº£ë‚«???ºÂ€?ë¸??ï¼œ?ëª„ìŠ‚.');
}
...
export function loadLocalReportState(): Partial<PersistedReportState> | null {
  ...
  try {
    ...
    logger.warn('localStorage ë¡œë“œ ?¤íŒ¨', { error: String(error) });
    return null;
  }
}
...
export function saveLocalReportState(state: PersistedReportState) {
  ...
  try {
    ...
    if (isQuotaError) {
      logger.warn('localStorage ?©ëŸ‰ ì´ˆê³¼', { error: String(error) });
    } else {
      logger.warn('localStorage ?€???¤íŒ¨', { error: String(error) });
    }
  }
}
...
function assertSupabase() {
  if (!supabase) {
    throw new Error('Supabase ?˜ê²½ë³€?˜ê? ?†ì–´ ?œë²„ ê³µìœ  ?€?¥ì„ ?¬ìš©?????†ìŠµ?ˆë‹¤.');
  }
  return supabase;
}
...
function getTemplateWorkbookSheetOrder(sheet: TemplateWorkbookSheet) {
  ...
  if (sheet.sheet_name === '2025???„ì²´') return 202500;
  if (sheet.sheet_name === '2026???„ì²´') return 202600;
  return 999999;
}
...
export function getStorageErrorMessage(error: unknown) {
  ...
  if (isSupabaseSchemaError(error)) {
    return `Supabase ?Œì´ë¸”ì„ ì°¾ì„ ???†ìŠµ?ˆë‹¤${code ? ` (${code})` : ''}. Supabase SQL Editor?ì„œ supabase/fix-42p01.sql???¤í–‰?˜ì„¸??`;
  }
  ...
  if (
    ...
    return 'Supabaseê°€ ?„ì§ P8 ?¤ì  ?€?¥ì„ ?ˆìš©?˜ì? ?ŠìŠµ?ˆë‹¤. supabase/add-p8-equipment.sql???¤í–‰?????¤ì‹œ ?€?¥í•˜?¸ìš”.';
  }

  if (code === '401' || code === '403') {
    return 'Supabase ?¸ì¦ ?¤ë¥˜?…ë‹ˆ?? ?˜ì´ì§€ë¥??ˆë¡œê³ ì¹¨?????¤ì‹œ ?œë„?´ì£¼?¸ìš”.';
  }

  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND' || /network/i.test(message)) {
    return '?¤íŠ¸?Œí¬ ?°ê²° ?¤ë¥˜?…ë‹ˆ?? ?¸í„°???°ê²°???•ì¸?????¤ì‹œ ?œë„?´ì£¼?¸ìš”.';
  }

  return `ê³µìœ  ?€???¤ë¥˜: ${message || '?????†ëŠ” ?¤ë¥˜'}`;
}
